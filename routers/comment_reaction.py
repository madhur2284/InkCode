...
from crud.comment_reaction import create_comment_reaction, delete_reaction, comment_reaction_notification
...
async def add_reaction(reaction_detail: CommentReactionCreate, db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user)) -> str:
    try:
        ...
        await comment_reaction_notification(db, current_user.avatar_url, current_user.username, current_user.id, reaction_detail.comment_id, reaction_detail.type)
