...
@router.post(path='/create', status_code=status.HTTP_201_CREATED, response_model=TagResposne)
async def create_tag(tag: TagCreate, db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
...
@router.delete(path='/delete/{slug}', status_code=status.HTTP_200_OK)
