...
async def post_reaction_notification(db: AsyncSession, avatar_url: str, username: str, user_id, post_id, type: str):
    post = await db.execute(select(Post).where(Post.id == post_id, Post.is_active==True, Post.status == 'published'))
    if not post:
    ...
    try:        
        await db.commit()
        await queue.put({"notification_id": notify.id, "type": notify.type, "avatar_url": avatar_url, "username": username, "user_id": user_id, "content": post.id, "message": f"{username} Liked your Post {post.title}" if type=='like' else f"{username} Disliked your Post {post.title}"})
    except Exception as e:
        await db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error: {e}")
